...
<div class="quick-note-wrapper">
  {#if expanded}
    ...
    <button
      ...
      class="w-full text-left bg-white dark:bg-stone-900 rounded-2xl border border-[#0D6E6E] dark:border-teal-500 p-4 cursor-pointer"
    >
      <!-- Meta -->
      <div class="flex items-center justify-between mb-2.5">
        ...
        <svg class="w-4 h-4 text-[#888888] dark:text-stone-500 transition-transform duration-200 rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {:else}
    ...
    >
      ...
      {#if needsExpand}
        <svg class="w-4 h-4 text-[#888888] dark:text-stone-500 flex-shrink-0 ml-auto transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
        </svg>
      {/if}
    </button>
